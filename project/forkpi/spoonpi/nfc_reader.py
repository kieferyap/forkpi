...
class NFCReader(object):
        ...
        try:
            ...
            if devices_found >= 1:
                ...
                try:
                    ...
                    while True:
                        self._poll_loop()
                        return self._card_uid.encode("hex")
                finally:
    ...
    def _poll_loop(self):
        ...
        nt = nfc.nfc_target()
        res = nfc.nfc_initiator_poll_target(self.__device, self.__modulations, len(self.__modulations), 0xff, 2,
                                            ctypes.byref(nt))
        # print "RES", res
        if res < 0:
            ...
            if uid:
                if not ((self._card_uid and self._card_present and uid == self._card_uid) \
                        and time.mktime(time.gmtime()) <= self._card_last_seen + self.card_timeout):
                    self._setup_device()
                    # self.read_card(uid)
            self._card_uid = uid
