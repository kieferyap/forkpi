...
def connect(device_name=None,baud=None,timeout=None):
    _ser = None
...
class FPS_GT511C3(SerialCommander):
    ...
    def Open(self):
        ...
        rp = self.GetResponse(response_size = 12 + 30)
        del packetbytes
        return rp.ACK
    ...
    def GetRawImage(self):
        ...
        packetbytes = cp.GetPacketBytes()
        self.SendCommand(packetbytes, 12)
        rp = self.GetResponse(response_size = 12 + 19200)
        retval = rp.ACK
        return retval
    ...
    def GetResponse(self, response_size=12):
        ...
        if rp.ACK:
            # delay(interval)
            r2 = bytearray(self._serial.read(response_size))
