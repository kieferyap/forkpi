...
mySerial->writebyte((uint8_t)(FINGERPRINT_STARTCODE >> 8));
mySerial->writebyte((uint8_t)FINGERPRINT_STARTCODE);
mySerial->writebyte((uint8_t)(addr >> 24));
mySerial->writebyte((uint8_t)(addr >> 16));
mySerial->writebyte((uint8_t)(addr >> 8));
mySerial->writebyte((uint8_t)(addr));
mySerial->writebyte((uint8_t)packettype);
mySerial->writebyte((uint8_t)(len >> 8));
mySerial->writebyte((uint8_t)(len));
...
uint8_t Pi_Fingerprint::getReply(uint8_t packet[], uint16_t timeout) {
  ...
  while (true) {
    ...
    // do {
    //   printf("Waiting for char (Timer: %d)\n", timer);
    ...
    // check packet!
    if (idx >= 9) {
      printf("Index is now more than 8!\n");
